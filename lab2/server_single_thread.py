...
def handle_request(client_socket, base_dir, client_address):
    try:
        ...
        print(f"Error receiving/decoding data: {e}")
        return
    ...
    try:
        ...
        print(f"Counter for {request_path}: {request_counter[request_path]}")
...
def start_server(port, base_dir):
    ...
    print(f"SINGLE-THREADED Server started on port {port}")
    ...
    try:
        while True:
            ...
            handle_request(client_socket, base_dir, address)
            client_socket.close()
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python server_single_threaded.py <directory>")
        sys.exit(1)

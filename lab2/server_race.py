...
def handle_request(client_socket, base_dir, client_address):
    try:
        ...
        print(f"[Thread {threading.current_thread().name}] Received request from {client_address}")
...
def start_server(port, base_dir):
    ...
    print(f"Server with RACE CONDITION started on port {port}")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python server_race.py <directory>")
        sys.exit(1)
